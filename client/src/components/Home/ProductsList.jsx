          ...
          return (
            <div className='card bg-global mt-3 p-0' key={e.name}>
              <div className='card-header text-white'>
                <Link to={`/products/${e.name}`} className='text-decoration-none text-white'>
                  <h5>{e.name}</h5>
                </Link>
              </div>
                        ...
                        <div className='px-2' key={p.name}>
                          <Link to={`/product/${p.id}`}>
                            <img
                              src={p.imageOffer}
                              alt={p.name}
                              className={images ? 'rounded-2' : 'img-fluid rounded-2'}
                              style={images ? { width: '320px', height: 'auto' } : null}
                            />
                          </Link>
                          {cont++}
                        </div>
