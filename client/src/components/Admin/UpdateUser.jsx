    ...
    <div>
      {data && (
        ...
        >
          {({ values, errors }) => (
            <Form
              className='container-sm my-0 bg-black p-4 text-start'
              style={{ maxWidth: '450px', borderRadius: '25px', border: ' solid 6px grey' }}
            >
              ...
              <div className='my-3 mx-auto'>
                <label htmlFor='password' className='text-white my-3 mx-5'>
              ...
              <div className='my-3 w-75 mx-auto'>
                ...
                <select className='form-select w-50 p-2' name='role' onChange={(e) => (values.role = e.target.value)}>
                  <option value={data.role === 'user' ? 'user' : 'admin'} className='outline-secondary'>
                    {data.role === 'user' ? 'Usuario' : 'Admin'}
                  </option>
                  <option value={data.role === 'user' ? 'admin' : 'user'} className='outline-secondary'>
                    {data.role === 'user' ? 'Admin' : 'Usuario'}
                  </option>
                </select>
              </div>
