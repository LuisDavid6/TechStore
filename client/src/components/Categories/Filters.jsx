    ...
    <div>
      <div className='mx-0' style={{ display: 'flex', justifyContent: 'center' }}>
        <div className='form-floating my-4'>
          <select
            className='form-select border-dark border-2 bg-black text-white'
            id='floatingSelect'
          ...
          <label className='text-white' for='floatingSelect'>
            Ordenar por:
          </label>
        </div>
      </div>
        ...
        <div className='btn-group-vertical' role='group' aria-label='Basic checkbox toggle vertical button group'>
          {categorySelect.subCategories &&
            categorySelect.subCategories.map((e) => {
              return (
                <button
                  className={`btn btn-outline-secondary text-white rounded-4 my-1 ${subCateg.includes(e.name) ? 'active' : null} `}
                  key={e.name}
                  onClick={() => handleSubCategory(e.name)}
                >
                  {e.name}
                </button>
              )
