...
export function addToCart(ids) {
  return async function (dispatch) {
    try {
      ...
      await axios.put(`/cart/addToCart/${userId}`, { productId })
      return dispatch({
        type: ADD_TO_CART,
      })
    } catch (e) {
...
export function removeFromCart(ids) {
  return async function (dispatch) {
    try {
      const { userId, productId } = ids
      await axios.put(`/cart/removeToCart/${userId}`, { productId })
      return dispatch({
...
export function updateUser(user, id) {
  return async function (dispatch) {
    try {
      await axios.put(`/users/update/${id}`, user)
      return dispatch({
        type: UPDATE_USER,
      })
    } catch (e) {
...
export function setShipmentStatus(info) {
  return async function (dispatch) {
    try {
      await axios.put('sales/shipment/changeStatus', info)
      return dispatch({
